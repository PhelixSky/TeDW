...
namespace Terraria.GameContent
{
  public class PortalHelper
  {
    ...
    public static int TryPlacingPortal(Projectile theBolt, Vector2 velocity, Vector2 theCrashVelocity)
    {
      ...
      for (int index = 0; index < (!flag ? PortalHelper.EDGES.Length : 2); ++index)
      {
        Point bestPosition;
    ...
    public static Color GetPortalColor(int player, int portal)
    {
      ...
      {
        float num = 0.08f;
        color = Main.hslToRgb((float) ((0.5 + (double) player * 0.159999996423721 + (double) portal * (double) num) % 1.0), 1f, 0.5f);
      }
      color.A = (byte) 66;
      return color;
    }
