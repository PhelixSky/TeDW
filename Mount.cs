...
namespace Terraria
{
  public class Mount
  {
      ...
      for (int index1 = 0; index1 < mountSpecificData.beams.Length; ++index1)
      {
        ...
        if (beam.cooldown == 0)
        {
          ...
          if (point16 != Point16.NegativeOne)
          {
            ...
            bool flag1;
            if (flag1 = mountedPlayer.whoAmI == Main.myPlayer)
            {
              bool flag2 = true;
      ...
      switch (this._type)
      {
        ...
        case 9:
          ...
          for (int index = 0; index < 2; ++index)
          {
            ...
            vector2_3 *= 14f;
            vector2_2 += vector2_3;
            Projectile.NewProjectile(vector2_2.X, vector2_2.Y, vector2_3.X, vector2_3.Y, 606, 100, 0.0f, Main.myPlayer, 0.0f, 0.0f);
          }
          break;
      }
    }
      ...
      switch (this._type)
      {
        ...
        case 8:
          if (state == 0 || state == 1)
          {
            ...
            {
              float num3 = (float) ((num1 + 1) * 16) - position.X;
              if ((double) num3 > (double) width)
                num3 = width;
              num2 += Collision.GetTileRotation(position) * num3;
              width -= num3;
              position.X += num3;
              ++num1;
            }
            float num4 = num2 / (float) mountedPlayer.width - mountedPlayer.fullRotation;
            float num5 = 0.0f;
            float num6 = 0.1570796f;
            if ((double) num4 < 0.0)
              num5 = (double) num4 <= -(double) num6 ? -num6 : num4;
        ...
        case 10:
          ...
          if (state == 1)
          {
            ...
            if (flag2)
            {
              ...
              {
                Dust dust = Main.dust[Dust.NewDust(Position, 0, 0, Utils.SelectRandom<int>(Main.rand, new int[3]{ 176, 177, 179 }), 0.0f, 0.0f, 0, new Color(), 1f)];
          ...
          if (flag1)
          {
            Dust dust = Main.dust[Dust.NewDust(mountedPlayer.position, mountedPlayer.width, mountedPlayer.height, Utils.SelectRandom<int>(Main.rand, new int[3]{ 176, 177, 179 }), 0.0f, 0.0f, 0, new Color(), 1f)];
        ...
        case 14:
          ...
          float num7 = (float) Math.Sign(mountedPlayer.velocity.X);
          float y = 12f;
          float num8 = 40f;
          ...
          if (flag3 && (double) velocity.Y == 0.0)
          {
            ...
            if (mountedPlayer.cMount == 0)
            {
              ...
              Vector2 vector2_1 = player1.position + new Vector2(num7 * 24f, 0.0f);
              player1.position = vector2_1;
              mountedPlayer.FloorVisuals(true);
              Player player2 = mountedPlayer;
              Vector2 vector2_2 = player2.position - new Vector2(num7 * 24f, 0.0f);
              player2.position = vector2_2;
            }
          }
          if ((double) num7 == (double) mountedPlayer.direction)
          {
            for (int index = 0; index < (int) (3.0 * (double) mountedPlayer.basiliskCharge); ++index)
            {
              ...
              Vector2 vector2 = mountedPlayer.Center + new Vector2(num7 * num8, y);
              dust.position = mountedPlayer.Center + new Vector2(num7 * (num8 - 2f), (float) ((double) y - 6.0 + (double) Main.rand.NextFloat() * 12.0));
    ...
    public void Draw(List<DrawData> playerDrawData, int drawType, Player drawPlayer, Vector2 Position, Color drawColor, SpriteEffects playerEffect, float shadow)
    {
      ...
      DrawData drawData = new DrawData(texture1, Position, new Rectangle?(rectangle1), drawColor, rotation1, origin, scale1, effect, 0);
