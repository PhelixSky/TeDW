...
namespace Terraria.IO
{
  public class WorldFile
  {
    ...
    {
      int[] pointers = new int[10]{ WorldFile.SaveFileFormatHeader(writer), WorldFile.SaveWorldHeader(writer), WorldFile.SaveWorldTiles(writer), WorldFile.SaveChests(writer), WorldFile.SaveSigns(writer), WorldFile.SaveNPCs(writer), WorldFile.SaveTileEntities(writer), WorldFile.SaveWeightedPressurePlates(writer), 0, 0 };
      ...
      for (int index1 = 0; index1 < Main.maxTilesX; ++index1)
      {
        ...
        for (int index2 = 0; index2 < Main.maxTilesY; ++index2)
        {
          ...
          if ((int) num4 > 1)
          {
            ...
            if (((int) num4 & 8) == 8)
              from.wire3(true);
            byte num6 = (byte) (((int) num4 & 112) >> 4);
            if ((int) num6 != 0 && Main.tileSolid[(int) from.type])
            {
              if ((int) num6 == 1)
                from.halfBrick(true);
              else
                from.slope((byte) ((uint) num6 - 1U));
            }
          }
          if ((int) num3 > 0)
          {
          ...
          if (index3 != -1)
          {
            if ((double) index2 <= Main.worldSurface)
            {
              ...
              {
                int num6 = (int) (Main.worldSurface - (double) index2 + 1.0);
                int num9 = num8 + 1 - num6;
                WorldGen.tileCounts[index3] += num6 * 5 + num9;
              }
            }
            else
    ...
    public static FileMetadata GetFileMetadata(string file, bool cloudSave)
    {
      ...
      try
      {
        ...
        bool flag;
        if (flag = cloudSave && SocialAPI.Cloud != null)
        {
          buffer = new byte[24];
          SocialAPI.Cloud.Read(file, buffer, 24);
        }
