...
label_58:
      ...
      for (int index1 = num7; index1 < num8; ++index1)
      {
        for (int index2 = num9; index2 < num10; ++index2)
        {
          ...
          if (tile != null && (int) tile.liquid > 0 && !tile.lava() && (index2 != num11 || !tile.active() || (!Main.tileSolid[(int) tile.type] || Main.tileSolidTop[(int) tile.type])))
          {
            ...
            int num13 = 16 - (int) ((double) num12 * 2.0);
        ...
        for (int index2 = num9; index2 < num10; ++index2)
        {
          if (Main.tile[index1, index2] != null)
          {
            if ((int) Main.tile[index1, index2].liquid > 0)
            {
              ...
              vector2_2.Y += num11 * 2f;
              int num12 = 16 - (int) ((double) num11 * 2.0);
              if ((double) vector2_1.X + (double) num1 > (double) vector2_2.X && (double) vector2_1.X < (double) vector2_2.X + 16.0 && ((double) vector2_1.Y + (double) num2 > (double) vector2_2.Y && (double) vector2_1.Y < (double) vector2_2.Y + (double) num12))
              {
      ...
      for (int index1 = num6; index1 < num7; ++index1)
      {
        for (int index2 = num8; index2 < num9; ++index2)
        {
          if (Main.tile[index1, index2] != null && (int) Main.tile[index1, index2].liquid > 0 && Main.tile[index1, index2].lava())
          {
            ...
            float num10 = (float) (256 - (int) Main.tile[index1, index2].liquid) / 32f;
            vector2.Y += num10 * 2f;
            int num11 = 16 - (int) ((double) num10 * 2.0);
            if ((double) Position.X + (double) Width > (double) vector2.X && (double) Position.X < (double) vector2.X + 16.0 && ((double) Position.Y + (double) num1 > (double) vector2.Y && (double) Position.Y < (double) vector2.Y + (double) num11))
              return true;
          }
        }
      }
        ...
        for (int j = num3; j < num4; ++j)
        {
          if (Main.tile[i, j] != null && (int) Main.tile[i, j].slope() == 0 && (!Main.tile[i, j].inActive() && Main.tile[i, j].active()) && ((int) Main.tile[i, j].type == 32 || (int) Main.tile[i, j].type == 37 || ((int) Main.tile[i, j].type == 48 || (int) Main.tile[i, j].type == 232) || ((int) Main.tile[i, j].type == 53 || (int) Main.tile[i, j].type == 57 || ((int) Main.tile[i, j].type == 58 || (int) Main.tile[i, j].type == 69)) || ((int) Main.tile[i, j].type == 76 || (int) Main.tile[i, j].type == 112 || ((int) Main.tile[i, j].type == 116 || (int) Main.tile[i, j].type == 123) || ((int) Main.tile[i, j].type == 224 || (int) Main.tile[i, j].type == 234 || (int) Main.tile[i, j].type == 352))))
          {
            ...
            if (type != 32 && type != 69 && (type != 80 && type != 352) && (type != 80 || !Main.expertMode))
            {
              ...
              else if ((double) vector2_1.X + (double) Width - 2.0 >= (double) vector2_2.X && (double) vector2_1.X + 2.0 <= (double) vector2_2.X + 16.0 && ((double) vector2_1.Y + (double) Height - 2.0 >= (double) vector2_2.Y && (double) vector2_1.Y + 2.0 <= (double) vector2_2.Y + (double) num6))
              {
                int num7 = 1;
                if ((double) vector2_1.X + (double) (Width / 2) < (double) vector2_2.X + 8.0)
                  num7 = -1;
                return new Vector2((float) num7, 15f);
              }
            }
        ...
        for (int index2 = num3; index2 < num4; ++index2)
        {
          if (Main.tile[index1, index2] != null && Main.tile[index1, index2].active() && !Main.tile[index1, index2].inActive())
          {
            if ((int) Main.tile[index1, index2].type == 51)
            {
              ...
              vector2_2.X = (float) (index1 * 16);
              vector2_2.Y = (float) (index2 * 16);
              if ((double) vector2_1.X + (double) Width > (double) vector2_2.X - 0.0 && (double) vector2_1.X < (double) vector2_2.X + 16.0 + (double) num5 && ((double) vector2_1.Y + (double) Height > (double) vector2_2.Y && (double) vector2_1.Y < (double) vector2_2.Y + 16.01))
              {
    ...
    public static void StepUp(ref Vector2 position, ref Vector2 velocity, int width, int height, ref float stepSpeed, ref float gfxOffY, int gravDir = 1, bool holdsMatching = false, int specialChecksMode = 0)
    {
      ...
      if (gravDir == 1)
      {
        ...
        bool flag9;
        flag8 = (((flag9 = flag5 && (tile4.nactive() && (!tile4.topSlope() || (int) tile4.slope() == 1 && (double) position.X + (double) (width / 2) < (double) (x * 16) || (int) tile4.slope() == 2 && (double) position.X + (double) (width / 2) > (double) (x * 16 + 16)) && (!tile4.topSlope() || (double) position.Y + (double) height > (double) (index1 * 16)) && (Main.tileSolid[(int) tile4.type] && !Main.tileSolidTop[(int) tile4.type] || holdsMatching && (Main.tileSolidTop[(int) tile4.type] && (int) tile4.frameY == 0 || TileID.Sets.Platforms[(int) tile4.type]) && ((!Main.tileSolid[(int) tile5.type] || !tile5.nactive()) && flag6)) || tile5.halfBrick() && tile5.nactive())) ? 1 : 0) & (!Main.tileSolidTop[(int) tile4.type] ? 1 : (!Main.tileSolidTop[(int) tile5.type] ? 1 : 0))) != 0;
      }
      else
      {
        Tile tile2 = Main.tile[x, index1 - gravDir];
    ...
    public static List<Point> GetEntityEdgeTiles(Entity entity, bool left = true, bool right = true, bool up = true, bool down = true)
    {
      ...
      if (y2 % 16 == 0)
        ++y2;
      int num1 = x2 / 16 - x1 / 16 + 1;
      int num2 = y2 / 16 - y1 / 16;
      List<Point> pointList = new List<Point>();
      int x3 = x1 / 16;
      int y3 = y1 / 16;
      for (int x4 = x3; x4 < x3 + num1; ++x4)
      {
        if (up)
          pointList.Add(new Point(x4, y3));
        if (down)
          pointList.Add(new Point(x4, y3 + num2));
      }
      for (int y4 = y3; y4 < y3 + num2; ++y4)
      {
        if (left)
          pointList.Add(new Point(x3, y4));
        if (right)
          pointList.Add(new Point(x3 + num1, y4));
      }
      return pointList;
    }
