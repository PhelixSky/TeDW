...
namespace Terraria
{
  public class NetMessage
  {
        ...
        for (int index2 = xStart; index2 < xStart + width; ++index2)
        {
          ...
          else
          {
            ...
            if ((int) num4 > 1)
            {
              ...
              if ((int) num6 != 0 && Main.tileSolid[(int) tile.type])
              {
                if ((int) num6 == 1)
                  tile.halfBrick(true);
                else
                  tile.slope((byte) ((uint) num6 - 1U));
              }
            }
            if ((int) num3 > 0)
            {
    ...
    private static void SyncOnePlayer(int plr, int toWho, int fromWho)
    {
      ...
      {
        NetMessage.SendData(14, toWho, fromWho, "", plr, (float) num, 0.0f, 0.0f, 0, 0, 0);
      ...
      {
        NetMessage.SendData(14, -1, plr, "", plr, 0.0f, 0.0f, 0.0f, 0, 0, 0);
        if (!Netplay.Clients[plr].IsAnnouncementCompleted)
          return;
