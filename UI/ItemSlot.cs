  ...
  public class ItemSlot
  {
    ...
    public static void RightClick(Item[] inv, int context = 0, int slot = 0)
    {
      ...
      else
      {
        ...
        {
          bool flag2 = true;
          if (context == 11 && (int) inv[slot].wingSlot > 0)
          {
    ...
    public static Color GetItemLight(ref Color currentColor, ref float scale, int type, bool outInTheWorld = false)
    {
      ...
      {
        if (ItemID.Sets.ItemIconPulse[type])
        {
          scale = Main.essScale;
    ...
    public static string GetGamepadInstructions(Item[] inv, int context = 0, int slot = 0)
    {
      ...
      if (context != 5 && context != 7)
      {
        ...
        if (num > 0)
        {
          ...
          if (num == 2)
            mouseItem = inv[slot];
          if (num != 1 || player.ItemSpace(mouseItem))
          {
      ...
      bool flag1 = false;
      if (context == 5)
        flag1 = Main.mouseItem.Prefix(-3) || Main.mouseItem.type == 0;
      if (context == 7)
        flag1 = Main.mouseItem.material;
      if (inv[slot].type > 0 && inv[slot].stack > 0)
      {
        if (Main.mouseItem.type > 0)
        {
      ...
      else if (Main.mouseItem.type > 0 && flag1)
        str1 += PlayerInput.BuildCommand(Lang.misc[65], 0 != 0, PlayerInput.ProfileGamepadUI.KeyStatus["MouseLeft"]);
      return str1;
    }
